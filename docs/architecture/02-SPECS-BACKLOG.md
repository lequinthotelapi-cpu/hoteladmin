...
| 11 | Venta POS transaccional | IN PROGRESS (código completo y probado, bug de doble IVA corregido; falta que despliegues y confirmes) | 03 | Alto (stock + caja) | [specs/SPEC-11-pos-venta-transaccional.md](specs/SPEC-11-pos-venta-transaccional.md) |
| 12 | Endurecimiento progresivo de `firestore.rules` | IN PROGRESS (bookings/guestAccounts/sales/invoices/inventoryMovements endurecidos y probados; products sigue sin endurecer — CRUD de catálogo sigue siendo del cliente, fuera de alcance) | Cada spec funcional ya VERIFIED (el usuario decidió no esperar) | Alto si se hace fuera de orden — bajo si se hace después de validar | [specs/SPEC-12-endurecer-firestore-rules.md](specs/SPEC-12-endurecer-firestore-rules.md) |
...
| 15 | Movimientos de inventario transaccionales | IN PROGRESS (Function desplegada y VERIFIED por el usuario en producción; firestore.rules de `inventoryMovements` endurecida en código, falta desplegarla) | — | Medio (stock, menos usado que reservas/POS pero comparte `products` con SPEC-11) | [specs/SPEC-15-inventario-movimientos.md](specs/SPEC-15-inventario-movimientos.md) |
